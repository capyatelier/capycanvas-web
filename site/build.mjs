...
function download(locale) {
  ...
  const pwa=`<section class="pwa" aria-labelledby="pwa-title"><h2 id="pwa-title">${e(p.title)}</h2><p>${e(p.intro)}</p><div class="pwa-browser" hidden>${osMenu}${select('browser',p.browser,p.browsers)}</div><div id="pwa-instructions" aria-live="polite">${Object.entries(p.fallback).map(([id,text])=>`<p data-pwa-fallback="${id}" hidden>${e(text)}</p>`).join('')}${guides}</div></section>`;
