...
import { shapes } from './illustration.mjs';
...
export async function references(e, record, directory) {
  ...
  await e.load('04-finished.capy'); await e.select('Hair');
  ...
  await e.load('04-finished.capy'); await e.select('Clothing shading');
  ...
  await e.select('Hair'); await e.invoke('auto_select');
  await e.lasso(shapes.hair); await e.invoke('auto_select');
  ...
  await e.lasso('M427 493 L540 493 L540 590 L427 590 Z');
  await e.invoke('scale_rotate');
  await record('tools-transforms', [tool, { selector: '#canvas', documentRect: [402, 430, 165, 190] }, { selector: '.layer-row.selected' }]);
  ...
  await e.setColor('#3e697b');
  await e.send({ type: 'color', action: { op: 'select', slot: 'background' } }); await e.setColor('#e6c0a3');
  ...
  await e.add('Shapes'); await e.brush(1, 14, '#ffffff');
  ...
  await b.call('DOM.setFileInputFiles', { nodeId: input.nodeId, files: [`${directory}/character.png`] });
  await e.wait('layerApp.state().layers.some(row=>row.label==="character.png")');
  await e.select('character.png');
  ...
  assert.ok(colored > 100000, 'Actual browser image import retains the colored portrait pixels');
  ...
  await e.select('Hair shading'); await e.brush(4, 55, '#383744');
